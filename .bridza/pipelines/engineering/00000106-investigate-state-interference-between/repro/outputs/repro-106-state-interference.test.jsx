...
// REPRO for task #106 — "state interference between two different tasks in
// task details". These tests are expected to FAIL on the current code: each
// one asserts the behaviour a user expects, and the failure IS the bug.
//
// The swap below is exactly what App.jsx does when you click another task in
// the sidebar while a task detail is open: it re-renders the SAME element
// position with a new `task` prop. TaskDetail has no `key`, so React reuses the
// instance and every piece of its useState that isn't explicitly reset carries
// task A's values into task B.
...
function mount(task) {
  ...
  act(() => host.root.render(<TaskDetail {...baseProps} task={task} />));
}
...
describe("#106 — switching tasks in the detail screen leaks the previous task's state", () => {
  ...
  it("E: the task-detail view mode is one global localStorage key, shared by every task and every tab", async () => {
    ...
    expect(localStorage.getItem("bridza.taskView")).toBe("canvas");
  ...
  it("F: two tasks that share an id in different pipelines keep each other's prompt, agent and model", async () => {
    ...
    document.body.appendChild(host);
    host.root = createRoot(host);
    act(() => host.root.render(<TaskDetail {...baseProps} pipeline={pipeA} task={same} />));
    await vi.waitFor(() => expect(api.getContext).toHaveBeenCalled());
    ...
    act(() => host.root.render(<TaskDetail {...baseProps} pipeline={pipeB} task={{ ...same, title: "Login form" }} />));
    await act(async () => { for (let i = 0; i < 10; i++) await Promise.resolve(); });
